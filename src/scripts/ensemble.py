...
def main(ont):
  save_path = f'../models/ensemble-{ont}.pt'
  ...
  for i in range(len(preds_val[0])):
    ...
    for j in range(len(preds_val)):
      aux_list.append((np.sum(preds_val[j][i]) > 0).astype(np.float32))
    mask_val.append(aux_list)
  ...
  for i in range(len(preds_test[0])):
    ...
    for j in range(len(preds_test)):
      aux_list.append((np.sum(preds_test[j][i]) > 0).astype(np.float32))
    mask_test.append(aux_list)
  mask_test = np.array(mask_test)

  input_val = torch.tensor(np.hstack(preds_val))
  mask_val = torch.tensor(mask_val).float()
  labels_val = torch.tensor(y_val).float()

  input_test = torch.tensor(np.hstack(preds_test))
  mask_test = torch.tensor(mask_test).float()
...
def predict_set(set_dataloader, model, device, set_name, ont):
  ...
  for i in y_pred:
    for j in i:
      f_pred.append(j)
  f_pred = np.array(f_pred)
  np.save(f'../preds/{ont}-ensemble-{set_name}.npy', f_pred)
